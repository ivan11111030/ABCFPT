...
export function SongManagementPanel({ songs, onImportSong, onAddSong, onUpdateSong, onDeleteSong }: SongManagementPanelProps) {
  ...
  const handleAddAllToSetlist = (category: "song" | "message" | "announcement") => {
    ...
    const itemsToAdd = categoryItems
      ...
      .map((s) => ({ itemId: s.id, itemCategory: category as const }));
...
export function SongManagementPanel({ songs, onImportSong, onAddSong, onUpdateSong, onDeleteSong }: SongManagementPanelProps) {
  ...
  const handleAddToSetlist = (songId: string) => {
    try {
      setlistMembershipStore.addItemToSetlist(songId, "song");
    } catch (error) {
      console.warn("Item already in setlist or error:", error);
    }
  };
  ...
  const handleRemoveFromSetlist = (songId: string) => {
    setlistMembershipStore.removeItemFromSetlist(songId, "song");
  };

  const handleAddAllToSetlist = () => {
    const itemsToAdd = songs
      .filter((s) => !setlistItems.has(s.id))
      .map((s) => ({ itemId: s.id, itemCategory: "song" as const }));
  ...
  return (
    <section className="song-panel">
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "2px solid var(--border)", marginBottom: 12 }}>
        <button
          ...
          style={{
            ...
            borderRadius: 0,
            background: "transparent",
            color: activeTab === "songs" ? "var(--accent)" : "var(--muted)",
            fontSize: 13,
          }}
        >
          Songs ({songs.length})
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 0,
            background: "transparent",
            color: activeTab === "actions" ? "var(--accent)" : "var(--muted)",
            fontSize: 13,
          }}
        >
          Setlist Actions
      ...
      {activeTab === "actions" && (
        <div style={{ background: "var(--panel)", borderRadius: 8, padding: 12, marginBottom: 12 }}>
          <h4 style={{ margin: "0 0 8px", fontSize: 13 }}>Bulk Setlist Operations</h4>
...
export function SongManagementPanel({ songs, onImportSong, onAddSong, onUpdateSong, onDeleteSong }: SongManagementPanelProps) {
  ...
  return (
    <section className="song-panel">
      ...
      {/* Setlist Actions Tab */}
      {activeTab === "actions" && (
        <div style={{ padding: "12px 0", marginBottom: 12, background: "var(--panel)", borderRadius: 8, padding: 12 }}>
          <h4 style={{ margin: "0 0 8px", fontSize: 13 }}>Bulk Setlist Operations</h4>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button
              type="button"
