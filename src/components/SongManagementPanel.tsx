...
export function SongManagementPanel({ songs, onImportSong, onAddSong, onUpdateSong, onDeleteSong }: SongManagementPanelProps) {
  ...
  const [setlistSongIds, setSetlistSongIds] = useState<Set<string>>(() => 
    new Set(setlistStore.getSetlist().map(item => item.songId))
  );
  ...
  useEffect(() => {
    const unsubscribe = setlistStore.subscribe(() => {
      setSetlistSongIds(new Set(setlistStore.getSetlist().map(item => item.songId)));
    });
    return unsubscribe;
  }, []);
    ...
    <section className="song-panel">
      ...
      <div className="song-list">
        {filteredSongs.map((song) => {
          const isInSetlist = setlistSongIds.has(song.id);
          const isSong = (song.category ?? "song") === "song";
          ...
          return (
            <article key={song.id} className="song-card">
              <div>
                ...
                <p>
                  {song.artist} • {isSong ? `${song.key}` : ""}
                  {isSong && song.tempo ? ` • ${song.tempo} BPM` : ""}
                  {isSong ? ` • ${song.slides.length} slides` : ` • ${song.slides.length} content items`}
                </p>
              </div>
              <div className="song-meta" style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                {isSong && isInSetlist && <span className="status" style={{ background: "var(--success)", color: "#fff" }}>In Setlist</span>}
                <span className="status section">{isSong ? song.currentSection : song.category === "message" ? "message" : "announcement"}</span>
                {isSong && (
                  <button 
                    type="button" 
                    className={`button ${isInSetlist ? "subtle" : "subtle"}`}
                    style={{ 
                      padding: "4px 10px", 
                      fontSize: 12,
                      background: isInSetlist ? "var(--success)" : "transparent",
                      color: isInSetlist ? "#fff" : "var(--text)",
                      border: isInSetlist ? "none" : "1px solid var(--border)"
                    }}
                    onClick={() => {
                      if (isInSetlist) {
                        setlistStore.removeFromSetlist(song.id);
                      } else {
                        setlistStore.addToSetlist(song.id);
                      }
                    }}
                  >
